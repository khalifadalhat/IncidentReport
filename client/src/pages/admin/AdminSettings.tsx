...
const AdminSettings: React.FC = () => {
  ...
  const handleCreateOrUpdateAgent = async () => {
    if (currentAgent) {
      ...
      await axios
        .put(`http://localhost:5000/agents/${currentAgent._id}`, newAgent)
        .then(response => {
          setAgents(
            agents.map(agent => (agent._id === currentAgent._id ? response.data.agent : agent))
          );
            ...
            <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700" key={agent._id}>
              ...
              <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white max-w-xs overflow-hidden overflow-ellipsis">
                <button
                  ...
                  className="bg-yellow-500 text-white mx-2 p-2 rounded">
                  Edit
                </button>
                <button
